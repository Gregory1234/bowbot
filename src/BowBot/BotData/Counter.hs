...
getCurrentCounterValue :: (HasCounter c d, MonadIO m, MonadReader r m, HasBotData d r) => c -> m Integer
getCurrentCounterValue c = do
  ...
  liftIO $ atomically $ do
    ...
    return $ c1 + c2
tryIncreaseCounter :: (HasCounter c d, MonadIO m, MonadReader r m, HasBotData d r) => c -> Integer -> m (Maybe Int)
...
clearCounter :: (HasCounter c d, MonadIO m, MonadReader r m, HasBotData d r) => c -> m ()
clearCounter c = do
  Counter {..} <- getCounter c
  liftIO $ atomically $ do
