...
infoCommand = Command CommandInfo
  ...
  , commandTimeout = 15
    ...
    let dcAccs = T.unlines $ map (\did -> let acc@DiscordAccount {..} = dc HM.! did in (if discordIsMember then "*" else " ") <> showDiscordAccount acc <> ", id " <> pack (show discordId) ) accountDiscords
    respond $ didYouMean <> renderedName <> ":\n" <> " - Bow Bot id: " <> pack (show accountId) <> "\n" <> " - Minecraft accounts:```\n" <> mcAccs <> "```" <> " - Discord accounts: ```\n" <> dcAccs <> "```"
