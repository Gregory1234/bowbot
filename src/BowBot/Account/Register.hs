...
createDummyBowBotAccount did = do
  ...
  liftIO $ flip runReaderT r $ withTransaction $ (either (const $ rollback $> Nothing) (pure . Just) =<<) $ runExceptT $ do
    c1 <- executeLogT [mysql|INSERT INTO `account`(`name`) VALUES ("Dummy Account")|]
    when (c1 <= 0) $ throwError ()
    bid <- BowBotId <$> insertID
    c3 <- executeLogT [mysql|INSERT INTO `account_discord`(`account_id`,`discord_id`) VALUES (bid, did)|]
...
addAltToBowBotAccount bid uuid = (>0) <$> executeLogT [mysql|INSERT INTO `account_minecraft`(`account_id`,`minecraft_uuid`,`type`,`selected`,`verified`) VALUES (bid,uuid, 'alt', 0, 0)|]